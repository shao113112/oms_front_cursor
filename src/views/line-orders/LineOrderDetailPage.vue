  ...
  <div v-if="order" class="max-w-full">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      ...
      <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">{{ order.orderNo }}</p>
      </div>
    </div>
    ...
    <div v-show="activeTab === 'basic'" class="space-y-6">
      ...
      <section class="page-card page-card-padding">
        ...
        <div class="grid grid-cols-3 gap-x-8 gap-y-4 text-sm">
          ...
          <div><span class="text-slate-500 block mb-0.5">计费体积(CBM)</span><div class="text-slate-800">{{ order.orderInfo?.billingVolume ?? order.billingVolume ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">参考号</span><div class="text-slate-800">{{ order.orderInfo?.referenceNo ?? order.referenceNo ?? '-' }}</div></div>
          <div class="col-span-3"><span class="text-slate-500 block mb-0.5">订单备注</span><div class="text-slate-800">{{ order.orderInfo?.orderRemark ?? order.memo ?? '-' }}</div></div>
        </div>
      </section>

      <!-- 箱 + 箱内商品 -->
      <section v-for="box in order.boxes" :key="box.id" class="page-card page-card-padding">
        <div class="flex items-center justify-between mb-4">
          <h2 class="section-title mb-0">箱 #{{ box.boxNo }}</h2>
        ...
        <div class="grid grid-cols-4 sm:grid-cols-4 gap-x-6 gap-y-4 text-sm mb-4">
          <div><span class="text-slate-500 block mb-0.5">箱类型</span><div class="text-slate-800">{{ box.boxType ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">长(CM)</span><div class="text-slate-800">{{ box.length ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">宽(CM)</span><div class="text-slate-800">{{ box.width ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">高(CM)</span><div class="text-slate-800">{{ box.height ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">重量(KG)</span><div class="text-slate-800">{{ box.weight ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">计费重(KG)</span><div class="text-slate-800">{{ box.billingWeight ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">体积(CBM)</span><div class="text-slate-800">{{ box.volume ?? '-' }}</div></div>
          <div><span class="text-slate-500 block mb-0.5">计费体积(CBM)</span><div class="text-slate-800">{{ box.billingVolume ?? '-' }}</div></div>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm min-w-[800px]">
            <thead class="bg-slate-50">
              <tr>
                <th class="px-4 py-2 text-left font-medium text-slate-700">商品图片</th>
                ...
                <th class="px-4 py-2 text-left font-medium text-slate-700">商品编码</th>
                ...
                <th class="px-4 py-2 text-left font-medium text-slate-700">申报价</th>
                <th class="px-4 py-2 text-left font-medium text-slate-700">单品重量(KG)</th>
              </tr>
            </thead>
    ...
    <!-- 订单费用 -->
    <div v-show="activeTab === 'fee'" class="page-card page-card-padding">
      <div class="flex items-center justify-between mb-4">
        <h2 class="section-title mb-0">订单费用</h2>
    ...
    <el-dialog v-model="showAddFeeDialog" title="添加费用" width="440px" destroy-on-close @close="resetFeeForm">
      <el-form :model="feeForm" label-width="90px">
        ...
        <el-form-item label="预计金额">
          <div class="w-full px-4 py-2 rounded-lg bg-slate-100 text-slate-600 text-sm">{{ feeExpectedAmountText }}</div>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 物流轨迹 -->
    <div v-show="activeTab === 'track'" class="page-card page-card-padding p-8 text-center text-slate-500">
      暂无物流轨迹
    </div>
  </div>
  </div>
  <div v-if="!loading && !order" class="py-12 text-center text-slate-500">
    订单不存在
